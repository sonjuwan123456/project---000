...
| `packages/core`    | 개념 모델 타입, 스키마, 명령 체계, 공용 수학 함수 | 수학 함수, 색상 토큰, 실시간 저장소 |
...
색은 `packages/core/src/palette.ts` 한 곳에서만 정의한다. CSS와 GLSL 양쪽에 손으로 적으면
반드시 어긋나기 때문이다. `cssVariables()`가 돌려주는 값을 `apps/web/src/main.tsx`가 첫 렌더
전에 `:root`에 심고, 셰이더는 같은 파일의 `categoryPalette()`를 쓴다. `global.css`에 남은
색 값은 변수가 붙기 전 첫 프레임용 대비값뿐이다.
